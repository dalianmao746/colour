...
dict_like, optional
    ...
    >>> range = np.linspace(10, 100, 10)
    >>> print(MultiSignal(range))
    [[   0.   10.]
     [   1.   20.]
     [   2.   30.]
     [   3.   40.]
    ...
    >>> print(MultiSignal(range, domain))
    [[  100.    10.]
     [  200.    20.]
     [  300.    30.]
     [  400.    40.]
    ...
    >>> range = tstack([np.linspace(10, 100, 10)] * 3)
    >>> range += np.array([0, 10, 20])
    >>> print(MultiSignal(range, domain))
    [[  100.    10.    20.    30.]
     [  200.    20.    30.    40.]
     [  300.    30.    40.    50.]
    ...
    >>> print(MultiSignal(dict(zip(domain, range))))
    [[  100.    10.    20.    30.]
     [  200.    20.    30.    40.]
     [  300.    30.    40.    50.]
     [  400.    40.    50.    60.]
    ...
    ...     data = dict(zip(['a', 'b', 'c'], tsplit(range)))
    ...     print(MultiSignal(  # doctest: +SKIP
    ...
    >>> x = 150
    >>> range = tstack([np.sin(np.linspace(0, 1, 10))] * 3)
    >>> range += np.array([0.0, 0.25, 0.5])
    >>> MultiSignal(range, domain)[x]  # doctest: +ELLIPSIS
...
dict_like
    ...
    def __repr__(self):
        ...
        >>> range = tstack([np.linspace(10, 100, 10)] * 3)
        >>> range += np.array([0, 10, 20])
        >>> MultiSignal(range)  # doctest: +ELLIPSIS
    ...
    def __getitem__(self, x):
        ...
        >>> range = tstack([np.linspace(10, 100, 10)] * 3)
        >>> range += np.array([0, 10, 20])
        >>> multi_signal = MultiSignal(range)
        >>> print(multi_signal)
        [[   0.   10.   20.   30.]
         [   1.   20.   30.   40.]
         [   2.   30.   40.   50.]
    ...
    def __eq__(self, other):
        ...
        --------
        >>> range = np.linspace(10, 100, 10)
        >>> multi_signal_1 = MultiSignal(range)
        >>> multi_signal_2 = MultiSignal(range)
    ...
    def arithmetical_operation(self, a, operator, in_place=False):
        ...
        if isinstance(a, MultiSignal):
            ...
            if a.ndim in (0, 1):
                ...
                assert a.shape[-1] == len(self._signals), (
                    'Operand "a" variable columns must have same count than '
                    'underlying "Signal" components!')

                for signal, a in zip(self._signals.values(), tsplit(a)):
                    signal.arithmetical_operation(a, operator, True)
...
dict_like, optional
        ...
        if isinstance(data, MultiSignal):
            ...
            for i, range_u in enumerate(data):
                signals[i] = Signal(range_u, domain)
            ...
            for i, range_u in enumerate(tsplit(range_u)):
                signals[i] = Signal(range_u, domain_u)
            ...
            if isinstance(data, Series):
                ...
                signals = OrderedDict(((label, Signal(
                    data[label], domain_u, name=label)) for label in data))
